...
| RECEPTIONNISTE | Réception de la série | `/reception/[serieId]` | `POST /api/series/[id]/reception` (one transaction) |
| RECEPTIONNISTE | Étiquettes | — | `GET /api/series/[id]/labels` (PDF) |
...
| 2 | 3 | Grouped reception, rules engine, temperatures, N° de contrôle NNNNN/AA, labels PDF with barcodes | A série received in one transaction; a failing rule shows its message; labels print with letters; TESTPLAN L2 |
