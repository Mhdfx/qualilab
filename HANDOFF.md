...
**2026-09-13 — the plan changed.** The old Firebird database (252 tables,
restored) and the four paper forms the lab fills by hand were compared with
this code: the base stands, the business core was modelled on a simplified
picture (one sample instead of one visit of ≈ 7 mixed-nature samples, 3 types
instead of 16 natures, a single limit instead of m / M criteria, one value
instead of one per unit, invented ids instead of the lab's yearly sequences).
**Phase 9** in `PLAN.md` holds the six workstreams (≈ 31 weeks); **`WORKFLOW.md`**
is the spec of the first one, the workflow. Everything built so far is kept.
...
src/
  ...
  components/
    layout/  RoleShell (one shell for every role: `navFor(role)` picks the
             nav, `activeHref` = longest matching prefix), DashboardShell,
             Sidebar, admin-nav / preleveur-nav / role-navs.ts
    preleveur/  VisitForm (header + LineEditor per line + recap), VisitDetail
                (arrival panel), visit-types.ts (line kinds, designation)
    ...
    admin/      ParametersManager, ProfilesManager, DocumentReferencesForm,
                LabSettingsForm, UsersManager, CatalogueManager, CompanyForm,
...
| 2026-09-09 | **Audit round 2 (5 relecteurs, verdict GO_WITH_NOTES) + recette de production** | Le round 1 avait introduit une régression : la revendication d'alerte (`alertsSentAt`) était posée AVANT la résolution des destinataires, donc un client sans adresse d'alerte était marqué alerté sans que rien ne parte, définitivement → destinataires résolus d'abord, claim relâchée sur exception, et `POST /api/samples/[id]/report/send` rejoue les alertes (le chemin de rattrapage n'existait pas). Deux pièges visibles en démonstration fermés : le panneau proposait « Approuver définitivement » à l'admin qui venait de signer l'étape 1 (409 garanti — remplacé par l'explication, et la page reste sur l'échantillon après approbation, là où sont le rapport et l'envoi) ; le conteneur tournait en UTC, donc **toutes** les heures affichées sur le VPS avaient une heure de retard → `TZ=Africa/Casablanca` sur l'image et le service, zone explicite dans `formatDateTime`/`formatDate` et deux formateurs partagés pour les composants client (supprime aussi un écart d'hydratation sur /qualite/temperatures). Également : rôle CLIENT retiré des rôles attribuables tant que `/portail` n'existe pas (`ASSIGNABLE_ROLES`, refusé par les deux routes utilisateurs) ; la fiche client 360 renvoyait le gestionnaire vers `/admin/factures/[id]` qui le rejetait — il reçoit le PDF que son rôle peut lire ; `requireRole` explicite sur réception et préleveur ; `GET /api/parameters` (les normes du labo) et `GET /api/lab-services` (les tarifs) fermés aux rôles qui ne les consomment pas ; longueurs de colonnes validées côté client et lieu (400, pas 500) ; `reject` répond 409 sur course comme validate/approve ; **suppression définitive de compte fermée** (`disabledPaths: ["/admin/remove-user"]`) car signatures et journal sont ON DELETE SET NULL ; lien feuille de paillasse en `prefetch={false}` (c'était un rendu Chromium par affichage de page en production) ; compteurs de la fiche client depuis des agrégats et non la page de 25 lignes ; `restore-db.sh` recrée la base et rejoue les migrations (un dump antérieur laissait des tables orphelines qui bloquaient le déploiement suivant) ; `vps-first-deploy.sh` s'arrête au lieu de prendre un échec de requête pour une base vide (le seed efface). **Ajout** : encaissement d'une facture client (`PATCH /api/invoices/[id]/payment`), le cycle s'arrêtait à « émise ». Recette : 99 contrôles en direct sur la production, 2 circuits complets, 137 tests |
| 2026-09-09 | **Recette navigateur en production** — circuit complet repris à la main dans le navigateur (préleveur → réception → technicien → validateur → direction → comptable → commercial), ce que les sondes d'API ne voyaient pas. Trois défauts client-visibles corrigés : (1) **les exposants au-delà de ³ tombaient des PDF** — l'image ne porte que `fonts-liberation`, qui couvre ¹ ² ³ (Latin-1) mais pas U+2074 et suivants, donc un seuil « 1.10⁴ UFC/g » s'imprimait « 1.10 UFC/g », faux d'un facteur dix mille sur un document conservé par le client → les exposants Unicode deviennent du balisage `<sup>` (aucun glyphe requis) dans `lib/html-text.ts`, désormais source unique de `escapeHtml`/`show` pour les trois générateurs, plus `fonts-dejavu-core` en filet ; (2) **feuille de paillasse datée de la veille** toute la journée — minuit à Casablanca est 23 h UTC la veille et le nom de fichier venait de `toISOString()` → `formatIsoDay()` lit le jour dans le fuseau du laboratoire (règle : ne jamais dériver une date locale d'un ISO UTC) ; (3) nombres non francisés à trois endroits (`formatDecimal()`). L'espace comptabilité annonçait encore en « prochaines fonctionnalités » les trois écrans de la phase 4 livrés — `nextSteps` devient optionnel. Constaté au passage : le journal d'audit a permis d'attribuer sans ambiguïté à un autre utilisateur connecté en parallèle la réouverture d'une facture pendant la recette — la traçabilité fait exactement son travail |
...
| 2026-09-14 | **Chantier 2 specified from the laboratory's own criteria (`CRITERES.md`).** The workbook « critère d'interprétation des résultats » confirms what the old database suggested: criteria are (product type × parameter × norm version) rows with a 3-class plan (n = 5, c, m, M), absence tests and M-only cases, and every product lists two norm versions. Decided: criteria as admin-edited data imported by a dry-run script (never retyped), one canonical parameter per germ with aliases, dated norm versions printed on the report, results per unit with one pure `interpret()` engine, client-specific product types, additive schema with the old `limitValue` as fallback. Rejected: hard-coding limits, one value per parameter, a spreadsheet kept as the source of truth. The workbook stays outside the public repo (client names) |
| 2026-09-14 | **The lab's first feedback on the new visit form: nine points, all already in the data model and on the PDF — the gap is the screen.** Decided: slice 1b before the recette, « le protocole tel quel » — one screen in the paper's order (every header field on the form, end-of-visit fields optional), the line type first so « Surface prélevée » exists on every line, n up to 50, the two « Analyses à effectuer » boxes stored on the série. Rejected: asking the lab to learn where the fields went (the paper is the acceptance test, rule of `WORKFLOW.md` §11); a second entry screen. Kept: the nature per line in the data (derived from the type on the form), the visit page panel for what is not typed on site. Four choices left to the lab (NEEDEDINFO Q21–Q24), none blocking |
| 2026-09-13 | **Phase 9 opened: the série becomes the unit of work; the sample stays the unit of analysis. Six design rules in `WORKFLOW.md`** | The client said « multiple things are missing, above all in the prélèvement ». Reading the old Firebird DB (200 072 samples in 27 406 orders, ≈ 7 per order; 16 natures; per-unit results; m/M criteria; yearly sequences 2772/26 and 20459/26) and the four paper forms showed the LIMS was modelled one sample at a time on three types. Chosen: one new aggregate `Serie` with **no status of its own** (derived from its samples — no second state machine), nature-driven line forms, everything picked or derived, one transaction per step, four correction verbs with a reason, additive migration with backfill. Rejected: porting the 252-table schema, a série state machine, free-text sites/places/products. Restore point tag `v1.0-avant-phase-9` + labelled VPS dump so the lab can go back if the new circuit is not what they want |
